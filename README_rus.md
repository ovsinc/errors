...
| -------- | --- | --------- | --------------------- |
| translateContext | `*TranslateContext` | Дополнительная информация для перевода. | `nil` |
| localizer  | `*i18n.Localizer` | Локализатор. Используется для выполнения переводов. | `nil` |

Для выполнения перевода обязательно требуется установить локализатор, используя функцию-параметр `SetLocalizer`.
...
Для ошибки `*Error` можно установить  с обязательной установкой контекст перевода.
В состав контекста перевода можно указать шаблон `TemplateData map[string]interface{}`, потребуется, если в сообщении необходимо осуществлять подстановки.
Для использования строки перевода при множественных значениях требуется установить число в `PluralCount interface{}`.
